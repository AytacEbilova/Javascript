...
window.addEventListener("load", () => {
  ...
  if (!localStorage.getItem("users")) {
    ...
    if (!localStorage.getItem("fav-product")) {
      localStorage.setItem("fav-product", JSON.stringify([]));
    }
  ...
  products.forEach((product) => {
    ...
    <a href="detail.html?id=${product.id}"><i class="fa-solid fa-circle-info">
                ...
                <button data-id=${
                 ...
                 <i class="fa-regular fa-heart btn btn-outline-danger heart-btn"></i>
    ...
    heartBtn.forEach((heartBtn) => {
      heartBtn.addEventListener("click", function () {
        ...
        const localFav = JSON.parse(localStorage.getItem("fav-product")) || [];
        const id = this.getAttribute("data-id");
        
        const checkProduct = localFav.find((x) => x.id == id);
        ...
        if (checkProduct === -1) {
          ...
          localFav.splice(checkProduct, 1);
        }
        localStorage.setItem("fav-product", JSON.stringify(localFav));
      });
    });
